	...
	$('select').change(function() {
		...
		if(sub_device_array != '') {
			...
			for(var i = 0; i < device_array.length;i++) {
				if(inArray(device_array[i],disabled_array)) { continue;}
...
function validate_submitbtn() {
		var radios = "<?php print $emp_status;?>";
		if(radios == 'Yes') { var select = covidien1; }
		else { var select = covidien2; }
        var empty = false;
...
<?php if($emp_status == "Yes"): ?>
					<table id="covidien-Yes" class="form-item-table-full" style="padding-left:0px">
					<tr>
						<td width="32%">
							<div class="label_left"><label><?php echo t('Business Unit:'); ?></label></div><div class="form-item-div"><div class="form-item-left"><span title="<?php echo t('This field is required.');?>" class="form-required">*</span></div><div><?php echo $business_unit;?></div></div>
						</td>
					</tr>
				</table>
<?php else: ?>				
					<table id="covidien-No" width="460px" class="form-item-table-full">
					<tr>
						<td width="280px">
...
<?php endif;?>				
			...
			<tr>
			<td colspan="3">
			<div style="padding-left : 10px; padding-bottom:5px;"><?php echo t('Default Product line and Role'); ?></div>
			<span title="<?php echo t('This field is required.');?>" class="form-required">*</span> <input type="text" readonly id="default_pl" id="default_pl" value="<?php echo $default_values['plvalue'];?>" style="width:125px" />
			 <span title="<?php echo t('This field is required.');?>" class="form-required">*</span> <input type="text" readonly id="default" id="default" value="<?php echo $default_values['role'];?>" style="width:125px" />
	...
	<div  class="add_user_message" id="toggleText" style="display: none; margin-top:-100px">
		...
		<ul>
			...
			<td width="20%" align="left">
				<input type="button" class="secondary_submit" onclick="if(!confirm('Are you sure you want to delete this user?')){return false;} else {window.location = '<?php print base_path(); ?>covidien/admin/users/<?php echo $form['nid']['#value']; ?>/delete';}" value="Delete this User" id="edit-add-new">
			</td>
				...
				<div class="device_tabs">
					...
					<?php 
						foreach($productline as $k=>$v) {
							...
							$pline = str_replace(" ","-",$k);
							if($k == $default_values['plvalue']) { $checked = " checked"; }
