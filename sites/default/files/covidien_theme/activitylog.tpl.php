...
$(document).ready(function() {
		...
		if(val == "") { $('#edit-last-name').val(Drupal.t('Enter user name')); }
			var val = $('#edit-isemp').val();
			if(val != "") { disablefun(val); }
			
	$('#edit-last-name').focus(function() {
		var val = $(this).val();
		if(val == Drupal.t("Enter user name")) { $('#edit-last-name').val(''); }
		else {}
	});
	$('#edit-isemp').change(function() { disablefun($(this).val()); });
	...
	function disablefun(val) {
		...
		if(val == "No") { $("#edit-bid, #edit-did").attr('disabled','disabled'); }
		else if(val == "Yes") { $("#edit-cid").attr('disabled','disabled'); }
	}	
});
</script>		
