...
function validate_submitbtn() {
        ...
        for(var i=0; i<text.length; i++) {
            ...
            if (($('#'+text[i]).val() == '') || ($.inArray(val, errormsg) != -1)) {
							if(text[i] != 'edit-default-role') { next = true; }
                empty = true;
            }
		}
        for(var i=0; i<select.length; i++) {
					if($('#'+ select[i]).val() == '') {
								if(select[i] != 'edit-roles') { next = true; }
                empty = true;
						}
		}
	...
	<table class="form-item-table-full" id="main1">
					...
					<tr>
						...
						<div style="margin-left : -5px;">
							<table width="100%" id="covidien-Yes" class="register_form form-item-table-full" style="padding-left:0px">
								<tr>
									<td width="32%">
										<div style="padding-left : 8px;"><label><?php echo t('Business Unit:'); ?></label></div><div class="form-item-div"><div class="form-item-left"><span title="<?php echo t('This field is required.');?>" class="form-required">*</span></div><div><?php echo $business_unit;?></div></div>
									</td>
								</tr>
							</table>
							<table width="460px" id="covidien-No" class="register_form form-item-table-full" style="display:none">
								<tr>
									<td style="padding-left:0px" width="267px">
		...
		<ul>
			...
			<input type="button" id="next" value="Next" class="form-submit secondary_submit non_active_grey">
</td>
						...
						<table style="width:75%">
							...
							<tbody>
								...
								foreach($productline as $k=>$v) {
									$sel = '';
									if($pl['selected'][$k] == 1) {$sel = "checked";}
									$pline = str_replace(" ","-",$k);
