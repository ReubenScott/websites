...
$(document).ready(function() {
		$(".iframe").colorbox({iframe:true, width:"500px", height: "500px", scrolling:false, onLoad: function() { $('#cboxClose').remove();}
});
...
<table class="form-item-table-full" style="width : 55%;">
		<tr>
			<td align="right" valign="top" style="padding-right : 18px;">
				<input type="button" class="form-submit secondary_submit" onclick="window.location = '<?php print $base_url; ?>/node/<?php print $sw;?>/edit';" value="Cancel" id="edit-add-new">
			</td>
		</tr>
</table>
</div>			
...
<script type="text/javascript">
		$(document).ready(function() {
			...
			if(hash != "") {
				if(hash=='#add') {
					$.colorbox({iframe:true, open:true, href:"<?php print $base_url; ?>/node/add/software-approval-unavailable/<?php print $id;?>", width:"500px", height: "500px", scrolling:false, overlayClose: false, onClosed:	function(){parent.location.reload();}, onLoad: function() { $('#cboxClose').remove();}
					});
				} else {
					var node = hash.split("/");
					$.colorbox({iframe:true, open:true, href:"<?php print $base_url; ?>/node/"+node[1]+"/edit/<?php print $id;?>", width:"500px", height: "500px", scrolling:false, overlayClose: false, onClosed:	function(){parent.location.reload();}, onLoad: function() { $('#cboxClose').remove();}
					});
				}
			}	
		});	
